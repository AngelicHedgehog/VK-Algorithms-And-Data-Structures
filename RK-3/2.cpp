...
auto isGraphEuler(int vertexCount, const ListGraph& graph) -> bool {
    ...
    while (!vertexQueue.empty()) {
        ...
        const auto& nextVertices = graph.GetNextVertices(vertexFrom);
        if (nextVertices.size() % 2 != 0) {
            return false;
        }

        for (const auto& vertexTo : nextVertices) {
...
auto run(std::istream& input, std::ostream& output) -> void {
    ...
    output << (isGraphEuler(vertexCount, graph) ? 1 : 0);
}
