...
void Array<T>::grow() {
  ...
  for (int i = 0; i < size_; ++i) new_buf[i] = buf_[i];
...
void Heap<T, Compare>::siftDown(int i) {
  ...
  if (left < arr.size() && cmp_(arr[left], arr[largest]) > 0) largest = left;
  if (right < arr.size() && cmp_(arr[right], arr[largest]) > 0) largest = right;
...
void Heap<T, Compare>::siftUp(int i) {
  while (i) {
    int parent = (i - 1) / 2;
    if (cmp_(arr[i], arr[parent]) <= 0) return;
...
void run(std::istream& input, std::ostream& output) {
  ...
  for (int i = 0; i < k; ++i) {
    ...
    for (int j = 0; j < n; ++j) input >> arrays[i][j];

    if (n) my_heap.insert({arrays[i][n - 1], n - 1, i});
  }
  ...
  for (int i = 0; i < res_len; ++i) output << res_arr[i] << ' ';

  for (int i = 0; i < k; ++i) delete[] arrays[i];
  delete[] arrays;
  delete[] res_arr;
}
